...
namespace WackyBag.Structures
{
	...
	public class SortedByOrder<TValue> : IEnumerable<TValue>
	{
		...
		public class SortedByOrderNode
		{
			...
			public readonly SortedByOrderNode? extend;
			/// <summary>
			/// 排序时的状态
			/// </summary>
			public enum State
			...
			/// </summary>
			public State state;
			public SortedByOrderNode(SortedByOrder<TValue> container, TValue Value, int index, int level)
			{
				Container = container;
				this.Value = Value;
				this.index = index;
			...
			public SortedByOrderNode(SortedByOrder<TValue> container, SortedByOrderNode extend) : this(container, extend.Value, extend.index, extend.level)
			{
				this.extend = extend;
			}
			/// <summary>
		...
		public SortedByOrder(SortedByOrder<TValue>? Parent = null)
		{
			if (Parent != null)
			{
				...
				{
					List<SortedByOrder<TValue>.SortedByOrderNode> l = [];
					//Nodes.Add(l);
					foreach (var j in i)
					{
						l.Add(new(this, j));
		...
		{
			SortedByOrder<TValue>.SortedByOrderNode res = new(this, value, CurrentNodes.Count, level);
		...
		where TKey : notnull
	{
		...
		public (int, int) Add(TKey key, TValue value)
		{
		...
		public (int, int) Add(OrderedValue orderedValue)
		{
		...
		public (int, int) AddOrdered<T>(T value)
