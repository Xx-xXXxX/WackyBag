...
namespace FixMath
{
    ...
    public partial struct Fix64 : IEquatable<Fix64>, IComparable<Fix64>
    {
        ...
        static readonly Fix64 Log2Max = new(LOG2MAX);
        static readonly Fix64 Log2Min = new(LOG2MIN);
        static readonly Fix64 Ln2 = new(LN2);
...
#pragma warning restore CS8765 // 参数类型的为 Null 性与重写成员不匹配(可能是由于为 Null 性特性)。
		{
            return obj is Fix64 v && v.m_rawValue == m_rawValue;
        }
...
@"
        ...
        {
            m_rawValue = ((long)value)<<FRACTIONAL_PLACES;
        }

        public static Fix64 operator <<(Fix64 a, int b) => new(a.m_rawValue << b);
		public static Fix64 operator >>(Fix64 a, int b) => new(a.m_rawValue >> b);
	}
}
